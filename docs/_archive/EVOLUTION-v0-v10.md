# Agent1st Protocol — Evolution History, v0 → v10 (archived)

Moved here from `docs/EVOLUTION.md` in v13.1. These are the exact transition notes for v0 → v1 through v9.2 → v10, kept byte-for-byte except for this header. Read them for the *why* behind the WHY layer, the validator, prompt-XML, and the §11 extensions.

**Section numbers are per-version.** `AGENTS.md §N` below refers to the numbering of that version's file — for v5.1 through v11 that is the eleven-principle file (§1 Role Contract … §4 Attention Engineering · §5 Semantic Hygiene · §6 CDD … §9 Delegation Design · §10 Semantic Logging · §11 Continuity). The key for all three numberings is at the top of `docs/EVOLUTION.md`. Validator output lines quoted below reflect the graph at that date.
...
- the v6 handoff from GPT-5.5-pro (pruned from `docs/handoffs/` in v8.1; its conclusions are curated below), framed explicitly as a working TZ rather than another opinion.
