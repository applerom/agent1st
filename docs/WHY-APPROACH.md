...
Edits stop being nearest-code edits. Refactors stop breaking invisible couplings. Handoffs stop starting from zero.
...
When you sit down in a project that uses this layer, the read order at session start is:
...
For any substantial edit:

- touch the graph before the code
- touch contracts before the code
- run validators before claiming done
...
## 8) Adopter's pattern — extending your project's `AGENTS.md`

Minimal Agent1st `AGENTS.md` is drop-in and protocol-only on purpose. Do not edit the core.
...
That header tells the agent what to pin. The rest of Agent1st stays untouched.
